    ...
    public RegisterResponse register(RegisterRequest request) {
        ...
        if (request != null) {
            ...
            if (!gender.equals("m") && !gender.equals("f")) {
                registerResult.setError(true);
                registerResult.setErrorMessage("The gender was invalid.");
                return registerResult;
            }
            UserAccount check = UserDAO.SINGLETON.getUser(username);
            if (check != null) {
                registerResult.setError(true);
                registerResult.setErrorMessage("That username is already in use.");
                return registerResult;
            }
            if (!registerResult.isError()) {
                ...
                if (!person.generateAncestors(4, person)) {
                    registerResult.setError(true);
                    registerResult.setErrorMessage("Internal server error.");
                    return registerResult;
                }
